...
def load_rules() -> dict:
    ...
    for extra in (RULES_EXTEND_FILE,):  # RULES_HAREDIM_FILE excluded
        if extra.exists():
            ext = json.loads(extra.read_text(encoding="utf-8"))
...
def build_context(zone: dict) -> dict[str, Any]:
    ...
    # Aliases expected by rules expressions
    ctx.setdefault("haredi_p", ctx.get("haredi_pct", 0))
    ctx.setdefault("traditional_p", ctx.get("traditional_pct", 0))
    ctx.setdefault("special_ed_p", ctx.get("special_education_pct", 0))
    ctx["plan_model"] = zone.get("plan_model") or ctx.get("settlement_type") or "B"
    return ctx
...
def population_form():
    return HTMLResponse((ROOT / "population_form.html").read_text(encoding="utf-8"))


@app.get("/rules-editor", response_class=HTMLResponse)
def rules_editor():
    return HTMLResponse((ROOT / "rules_editor.html").read_text(encoding="utf-8"))


@app.get("/rules-editor-v2", response_class=HTMLResponse)
def rules_editor_v2():
    return HTMLResponse((ROOT / "rules_editor_v2.html").read_text(encoding="utf-8"))


@app.get("/api/rules-source")
...
def get_zones():
    ...
    if not zones_file.exists():
        raise HTTPException(status_code=404, detail="zones_data.json not found")
